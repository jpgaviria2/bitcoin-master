...
class ExampleTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        # Get node 1 to mine an additional block and send it to node 2 and verify node 2 received it
        self.log.info("Clean previous connections")
        self.nodes[0].disconnect_p2ps()
        self.disconnect_nodes(1,2)
        self.log.info("Open a new connection with node 0")
        peer_messaging = self.nodes[0].add_p2p_connection(BaseNode())

        
        self.log.info("Mining an additional block")
        block = create_block(self.tip, create_coinbase(height+1), self.block_time)
        block.solve()
        block_message = msg_block(block)
        # Send message is used to send a P2P message to the node over our P2PInterface
        ...
        with p2p_lock:
            for block in peer_receiving.block_receive_map.values():
                assert_equal(block, 1) # compare each block and ensure they are the same
